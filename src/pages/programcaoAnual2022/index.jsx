...
function ProgramacaoAnual2022() {
    var token = window.localStorage.getItem("x-access-token");
    const navigate = useNavigate();

    const [loading, setLoading] = useState(false);
    const [openAddProgramacaoAnual2022, setOpenAddProtocolo] = useState(false);
    const [titulo, setTitulo] = useState("");
    const [link, setLink] = useState("");
    const [programacaoAnual2022, setProtocolos] = useState([]);
    ...
    async function handleAdicionar() {
        ...
        await axios({
            ...
            .then((res) => {
                if (!res.data.permissoes.includes("ProgramacaoAnual2022")) {
                    navigate("/");
                    return Swal.fire("Acesso negado", "", "error");
                }
                axios({
                    method: "post",
                    url: "https://central-pm-api-dev.onrender.com/programacaoAnual2022/",
                    data: {
                        titulo: titulo,
                        link: link,
                    },
    ...
    function getProgramacaoAnual2022() {
        axios({
            method: "get",
            url: "https://central-pm-api-dev.onrender.com/programacaoAnual2022",
            headers: {
        ...
        if (confirmacao) {
            axios({
                ...
                .then((res) => {
                    ...
                    axios({
                        method: "delete",
                        url: `https://central-pm-api-dev.onrender.com/programacaoAnual2022/${id}`,
                        headers: {
                            authorization: token,
                        },
                    })
