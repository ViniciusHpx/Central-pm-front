...
function ProgramacaoAnual2024() {
    var token = window.localStorage.getItem("x-access-token");
    const navigate = useNavigate();

    const [loading, setLoading] = useState(false);
    const [openAddProgramacaoAnual2024, setOpenAddProtocolo] = useState(false);
    const [titulo, setTitulo] = useState("");
    const [link, setLink] = useState("");
    const [programacaoAnual2024, setProtocolos] = useState([]);
    ...
    async function handleAdicionar() {
        ...
        await axios({
            ...
            .then((res) => {
                if (!res.data.permissoes.includes("ProgramacaoAnual2024")) {
                    navigate("/");
                    return Swal.fire("Acesso negado", "", "error");
                }
                axios({
                    method: "post",
                    url: "https://central-pm-api-dev.onrender.com/programacaoAnual2024/",
                    data: {
                        titulo: titulo,
                        link: link,
                    },
    ...
    function getProgramacaoAnual2024() {
        axios({
            method: "get",
            url: "https://central-pm-api-dev.onrender.com/programacaoAnual2024",
            headers: {
        ...
        if (confirmacao) {
            axios({
                ...
                .then((res) => {
                    ...
                    axios({
                        method: "delete",
                        url: `https://central-pm-api-dev.onrender.com/programacaoAnual2024/${id}`,
                        headers: {
                            authorization: token,
                        },
                    })
