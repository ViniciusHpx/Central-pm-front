...
function PlanoMunicipalDeSaude() {
    var token = window.localStorage.getItem("x-access-token");
    const navigate = useNavigate();

    const [loading, setLoading] = useState(false);
    const [openAddPlanoMunicipalDeSaude, setOpenAddProtocolo] = useState(false);
    const [titulo, setTitulo] = useState("");
    const [link, setLink] = useState("");
    const [planoMunicipalDeSaude, setProtocolos] = useState([]);
    ...
    async function handleAdicionar() {
        ...
        if (titulo.length < 3) {
            setLoading(false);
            return Swal.fire(
                "Plano Municipal precisa ser maior que 3 caracteres",
                "",
                "error"
            );
        }
        ...
        await axios({
            ...
            .then((res) => {
                if (!res.data.permissoes.includes("PlanoMunicipalDeSaude")) {
                    navigate("/");
                    return Swal.fire("Acesso negado", "", "error");
                }
                axios({
                    method: "post",
                    url: "https://central-pm-api-dev.onrender.com/planomunicipaldesaude/",
                    data: {
                        titulo: titulo,
                        link: link,
                    },
    ...
    function getPlanoMunicipalDeSaude() {
        axios({
            method: "get",
            url: "https://central-pm-api-dev.onrender.com/planomunicipaldesaude",
            headers: {
    ...
    function handleDeletar(id) {
        ...
        if (confirmacao) {
            axios({
                method: "delete",
                url: `https://central-pm-api-dev.onrender.com/planomunicipaldesaude/${id}`,
                headers: {
                    authorization: token,
                },
            })
                ...
                .catch((err) => {
                    return Swal.fire("Erro ao excluir cadastro", "", "error");
                });
        }
    }
                    ...
                    <div>
                        ...
                        >
                            <ol className="list-group list-group-numbered">
                                {planoMunicipalDeSaude.map((item) => {
                                    return (
