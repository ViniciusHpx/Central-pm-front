...
function Rag2025() {
    ...
    const [openAddRag2025, setOpenAddProtocolo] = useState(false);
    const [titulo, setTitulo] = useState("");
    const [link, setLink] = useState("");
    const [rag2025, setProtocolos] = useState([]);
    ...
    async function handleAdicionar() {
        ...
        if (titulo.length < 3) {
            setLoading(false);
            return Swal.fire(
                "Texto precisa ser maior que 3 caracteres",
                "",
                "error"
            );
        }
        ...
        await axios({
            ...
            .then((res) => {
                if (!res.data.permissoes.includes("Rag2025")) {
                    navigate("/");
                    return Swal.fire("Acesso negado", "", "error");
                }
                axios({
                    method: "post",
                    url: "https://central-pm-api-dev.onrender.com/rag2025/",
                    data: {
                        titulo: titulo,
                        link: link,
                    },
        ...
        if (confirmacao) {
            axios({
                ...
                .then((res) => {
                    ...
                    axios({
                        method: "delete",
                        url: `https://central-pm-api-dev.onrender.com/rag2025/${id}`,
                        headers: {
                            authorization: token,
                        },
                    })
